import type { UIRunState, UITimelineItem } from '@/app/components/ui/types';
...
export function reduceEvent(state: UIRunState, incoming: any): UIRunState {
  ...
  if (incoming.type === 'design_update' && incoming.data?.design) {
    return {
      ...s,
      design: incoming.data.design,
    };
  }

  if (incoming.type === 'plan_proposal' && incoming.data?.plan) {
    const plan = incoming.data.plan;
  ...
  if (incoming.type === 'plan_approved' || incoming.type === 'plan_rejected') {
    return {
      ...s,
      plan: undefined,
    };
    ...
    switch (item?.type) {
      ...
      case 'tool_call_item': {
        ...
        s = {
          ...
          _meta: { ...s._meta, liveMessageId: undefined },
        };
...
function handleRawModel(state: UIRunState, data: any): UIRunState {
  ...
  if (isTextDone(data)) {
    const liveId = s._meta.liveMessageId;
    if (liveId) {
      const prev = s.messages.find((m) => m.id === liveId);
      if (prev) {
